...
// Need to define "nitroExternCThreadFunctionType" to avoid warning on some
...
#if defined(NITRO_USE_PTHREADS) || defined(NITRO_HP_PTHREADS)
#include <pthread.h>
extern "C" { typedef void *(*nitroExternCThreadFunctionType)(void *); }
#else
//typedef nitroThreadFunctionType nitroExternCThreadFunctionType;
#endif
  ...
  int MultiThreader::GetGlobalDefaultNumberOfThreads()
  {
    if (MultiThreaderGlobalDefaultNumberOfThreads == 0)
      ...
      if (result == -1)
        ...
        // Lets limit the number of threads to NITRO_MAX_THREADS
        if (num > NITRO_MAX_THREADS)
          {
          num = NITRO_MAX_THREADS;
          }
...
// required user defined methods
                                         ...
                                         ThreadFunctionType f, void *data )
  {
    ...
    if ( index >= this->m_NumberOfThreads )
      {
      nitroErrorMacro( << "Can't set method " << index <<
...
// Execute the method set as the SingleMethod on NumberOfThreads threads.
  void MultiThreader::SingleMethodExecute()
  {
    ...
    DWORD              threadId;
    HANDLE             process_id[NITRO_MAX_THREADS];
#endif

#ifdef NITRO_USE_SPROC
    siginfo_t          info_ptr;
    int                process_id[NITRO_MAX_THREADS];
#endif

#ifdef NITRO_USE_PTHREADS
    pthread_t          process_id[NITRO_MAX_THREADS];
#endif

    if ( !this->m_SingleMethod )
      {
      nitroErrorMacro( << "No single method set!" );
      return;
      }
    ...
    for (thread_loop = 1; thread_loop < this->m_NumberOfThreads; thread_loop++ )
      ...
      if (process_id[thread_loop] == NULL)
        {
        nitroErrorMacro("Error in thread creation !!!");
        }
      }
    ...
    for ( thread_loop = 1; thread_loop < this->m_NumberOfThreads; thread_loop++ )
      ...
      if ( process_id[thread_loop] == -1)
        {
        nitroErrorMacro("sproc call failed. Code: " << errno << endl);
        }
      }
    ...
    for ( thread_loop = 1; thread_loop < this->m_NumberOfThreads; thread_loop++ )
      ...
      threadError =
        pthread_create( &(process_id[thread_loop]), &attr,
                        reinterpret_cast<nitroExternCThreadFunctionType>(
                          this->m_SingleMethod),
                        ( (void *)(&this->m_ThreadInfoArray[thread_loop]) ) );
      if (threadError != 0)
        {
        nitroErrorMacro(<< "Unable to create a thread.  pthread_create() returned "
                        << threadError);
        }
#endif
      }
  ...
  void MultiThreader::MultipleMethodExecute()
  {
    ...
    for ( thread_loop = 0; thread_loop < this->m_NumberOfThreads; thread_loop++ )
      ...
      if ( this->m_MultipleMethod[thread_loop] == (ThreadFunctionType)NULL)
        {
        nitroErrorMacro( << "No multiple method set for: " << thread_loop );
        return;
        }
      }
  ...
  int MultiThreader::SpawnThread( ThreadFunctionType f, void *userdata )
  {
    ...
    for ( id = 0; id < NITRO_MAX_THREADS; id++ )
      {
    ...
    if ( id >= NITRO_MAX_THREADS )
      {
      nitroErrorMacro( << "You have too many active threads!" );
      return -1;
      }
    ...
    pthread_create( &(this->m_SpawnedThreadProcessID[id]),
                    &attr,
                    reinterpret_cast<nitroExternCThreadFunctionType>(f),
                    ( (void *)(&this->m_SpawnedThreadInfoArray[id]) ) );
#endif
    ...
    nitroErrorMacro( << "Cannot spawn thread in a single threaded environment!" );
  ...
  void MultiThreader::TerminateThread( int threadID )
  {
    ...
    // There is no multi threading, so there is only one thread.
    // This won't work - so give an error message.
    nitroErrorMacro(<< "Cannot terminate thread in single threaded environment!");
#endif
#endif
#endif
...
//----------------------------------------------------------------------------
  ...
  {
#if defined(NITRO_USE_PTHREADS)
    return pthread_self();
#elif defined(NITRO_USE_WIN32_THREADS)
    return GetCurrentThreadId();
#elif defined(NITRO_USE_SPROC)
    return getpid();
#else
...
#elif defined NITRO_USE_WIN32_THREADS
      "WIN32 Threads"
#elif defined NITRO_HP_PTHREADS
      "HP PThreads"
#else
      "NO THREADS SUPPORT"
#endif
