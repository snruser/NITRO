...
#if defined(NITRO_USE_PTHREAD) || defined(NITRO_HP_PTHREAD)
...
// If NITRO_USE_SPROC is defined, then sproc() will be used to create
// multiple threads on an SGI. If NITRO_USE_PTHREAD is defined, then
// pthread_create() will be used to create multiple threads (on
// a sun, for example)

// Defined in nitroSystemIncludes.h:
//   NITRO_MAX_THREADS

// If NITRO_USE_PTHREADS is defined, then the multithreaded
